 ...
 * It hashes both inputs with SHA-256 before comparing to handle variable lengths.
 */
...
<body style="font-family: Georgia, serif; max-width: 600px; margin: 0 auto; padding: 24px; background: #fff; color: #333;">
  ...
  app.get("/api/dashboard", (req, res) => {
    ...
    // Fail securely if no password is configured or if the comparison fails
    if (!password || !timingSafeCompare(authHeader, `Bearer ${password}`)) {
