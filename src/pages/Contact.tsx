...
export default function Contact() {
  ...
  const Field = ({ id, label, required, error }: { id: string; label: string; required?: boolean; error?: string }) => (
    <div className="space-y-1.5">
      <label htmlFor={id} className="text-xs font-bold text-stone-600 uppercase tracking-wider block">
        {label} {required && <span className="text-brand-primary" aria-hidden="true">*</span>}
        {required && <span className="sr-only">(required)</span>}
      </label>
      {error && touched[id] && (
        <span id={`${id}-error`} role="alert" className="text-red-500 text-xs block">
          {error}
        </span>
      )}
    </div>
