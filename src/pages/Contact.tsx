...
export default function Contact() {
  ...
  const Field = ({ id, label, required, error }: { id: string; label: string; required?: boolean; error?: string }) => (
    <div className="flex flex-col space-y-1.5">
      <label htmlFor={id} className="text-xs font-bold text-stone-600 uppercase tracking-wider cursor-pointer">
        {label} {required && <span className="text-brand-primary">*</span>}
      </label>
      {error && touched[id] && (
        <span id={`${id}-error`} role="alert" className="text-red-500 text-xs">
          {error}
        </span>
      )}
    </div>
          ...
          <div className="md:col-span-3">
            <div className="bg-brand-secondary border border-purple-100 rounded-3xl p-8 shadow-sm">
              ...
              <form onSubmit={handleSubmit} className="space-y-5" noValidate>
                ...
                {/* Subject */}
                <div className="space-y-1.5">
                  <label htmlFor="subject" className="text-xs font-bold text-stone-600 uppercase tracking-wider block cursor-pointer">Subject</label>
                  <input
                    id="subject"
                    type="text"
                ...
                {/* Support type */}
                <div className="space-y-1.5">
                  <label htmlFor="supportType" className="text-xs font-bold text-stone-600 uppercase tracking-wider block cursor-pointer">What do you need help with?</label>
                  <select
                    id="supportType"
